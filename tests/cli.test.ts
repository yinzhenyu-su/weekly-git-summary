...
describe('CLI Tool Tests', () => {
  ...
  it('should handle date parameters', () => {
    ...
    const result = execSync(
      `node build/cli.js --dir ${testDir} --since ${since} --until ${until} --json`,
      {
        encoding: 'utf8',
        cwd: process.cwd(),
      },
  ...
  it('should handle author filter', () => {
    try {
      const result = execSync(
        `node build/cli.js --dir ${testDir} --author "Test User" --json`,
        {
          encoding: 'utf8',
          cwd: process.cwd(),
          stdio: 'pipe',
      ...
      if (jsonLine) {
        ...
        // 检查是否有提交记录
        if (
          jsonResult.repositories.length > 0
          && jsonResult.repositories[0].commits.length > 0
        ) {
          const commits = jsonResult.repositories[0].commits.flatMap(
            (day: any) => day.commits,
          )
  ...
  it('should handle author names with spaces using double quotes', () => {
    ...
    execSync('git add .', { cwd: testDir })
    execSync('git commit -m "Test commit with spaced author"', {
      cwd: testDir,
    })

    try {
      const result = execSync(
        `node build/cli.js --dir ${testDir} --author "John Doe" --json`,
        {
          encoding: 'utf8',
          cwd: process.cwd(),
          stdio: 'pipe',
  ...
  it('should handle author names with spaces using single quotes', () => {
    ...
    execSync('git add .', { cwd: testDir })
    execSync('git commit -m "Test commit with Chinese spaced author"', {
      cwd: testDir,
    })

    try {
      const result = execSync(
        `node build/cli.js --dir ${testDir} --author 'Zhang San' --json`,
        {
          encoding: 'utf8',
          cwd: process.cwd(),
          stdio: 'pipe',
  ...
  it('should handle long form author parameter with spaces', () => {
    try {
      const result = execSync(
        `node build/cli.js --dir ${testDir} --author "Li Ming Wang" --json`,
        {
          encoding: 'utf8',
          cwd: process.cwd(),
          stdio: 'pipe',
  ...
  it('should handle author names with backslash escaped spaces', () => {
    ...
    execSync('git add .', { cwd: testDir })
    execSync('git commit -m "Test commit with backslash escaped author"', {
      cwd: testDir,
    })
    ...
    try {
      // 测试引号内的反斜杠转义
      const result = execSync(
        `node build/cli.js --dir ${testDir} --author "Jane\\ Smith" --json`,
        {
          encoding: 'utf8',
          cwd: process.cwd(),
          stdio: 'pipe',
  ...
  it('should handle author names with multiple backslash escaped spaces', () => {
    ...
    execSync('git add .', { cwd: testDir })
    execSync('git commit -m "Test commit with multiple spaced author"', {
      cwd: testDir,
    })
    ...
    try {
      // 测试多个反斜杠转义空格
      const result = execSync(
        `node build/cli.js --dir ${testDir} --author "Dr\\ John\\ Doe\\ Jr" --json`,
        {
          encoding: 'utf8',
          cwd: process.cwd(),
          stdio: 'pipe',
  ...
  it('should handle directory paths with backslash escaped spaces', () => {
    ...
    try {
      // 测试反斜杠转义的目录路径
      const result = execSync(
        `node build/cli.js --dir "test\\ spaced\\ repo" --json`,
        {
          encoding: 'utf8',
          cwd: process.cwd(),
          stdio: 'pipe',
  ...
  it('should handle directory paths with multiple backslash escaped spaces', () => {
    ...
    try {
      // 测试多个反斜杠转义的目录路径
      const result = execSync(
        `node build/cli.js --dir "my\\ test\\ project\\ folder" --json`,
        {
          encoding: 'utf8',
          cwd: process.cwd(),
          stdio: 'pipe',
  ...
  it('should handle multiple authors filtering', () => {
    ...
    try {
      // 测试多个作者过滤
      const result = execSync(
        `node build/cli.js --dir ${testDir} -a "Alice Smith" -a "Bob Johnson" --json`,
        {
          encoding: 'utf8',
          cwd: process.cwd(),
          stdio: 'pipe',
  ...
  it('should handle multiple authors with spaces and backslash escaping', () => {
    ...
    try {
      ...
      const result = execSync(
        `node build/cli.js --dir ${testDir} -a "Dr.\\ John\\ Doe\\ Jr" --author "Mary\\ Jane\\ Watson" --json`,
        {
          encoding: 'utf8',
          cwd: process.cwd(),
          stdio: 'pipe',
  ...
  it('should handle non-existent directory', () => {
    ...
    try {
      const result = execSync(
        `node build/cli.js --dir ${nonExistentDir} --json`,
        {
          encoding: 'utf8',
          cwd: process.cwd(),
          stdio: 'pipe', // 隐藏错误输出
