...
test('shell --help', () => {
  ...
  const { stdout, exitCode } = Bun.spawnSync({
    cmd: ['bash', '../weekly-git-summary.sh', '--help'],
    cwd: __dirname,
    stdout: 'pipe',
  })
  // 断言脚本的退出码
...
test('shell --dir --since --until', () => {
  const { stdout, exitCode } = Bun.spawnSync({
    cmd: ['bash', '../weekly-git-summary.sh', '--dir', '..', '-s', '2024-01-01', '-u', '2025-04-17'],
    cwd: __dirname,
    stdout: 'pipe',
  })
  expect(exitCode).toBe(0)
...
test('shell --dir --since --util --json', () => {
  const { stdout, exitCode } = Bun.spawnSync({
    cmd: ['bash', '../weekly-git-summary.sh', '--dir', '..', '-s', '2024-01-01', '-u', '2025-04-17', '--json'],
    cwd: __dirname,
    stdout: 'pipe',
  })
  expect(exitCode).toBe(0)
...
test('shell --dir --since --util --md', () => {
  const { stdout, exitCode } = Bun.spawnSync({
    cmd: ['bash', '../weekly-git-summary.sh', '--dir', '..', '-s', '2024-01-01', '-u', '2025-04-17', '--md'],
    cwd: __dirname,
    stdout: 'pipe',
  })
  expect(exitCode).toBe(0)
...
test('shell --dir --since --util --author', () => {
  const { stdout, exitCode } = Bun.spawnSync({
    cmd: ['bash', '../weekly-git-summary.sh', '--dir', '..', '-s', '2024-01-01', '-u', '2025-04-17', '--author', 'notexist'],
    cwd: __dirname,
    stdout: 'pipe',
  })
  expect(exitCode).toBe(0)
